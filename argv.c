...
char **argv(char *line, int argument_count)
{
	...
	ret_newline = malloc(sizeof(char *) * (argument_count + 1));
	if (ret_newline == NULL)
	{
		perror("Error");
	}
	...
	while (tokenizer != NULL)
	{
		...
		ret_newline[i] = malloc(sizeof(char) * (len_line + 1));
		if (ret_newline[i] == NULL)
		{
			free_double_pointer(ret_newline, argument_count);
			perror("Error");
		}

		ret_newline[i] = _strcpy(ret_newline[i], tokenizer);
